...
2. **Honesty over completeness** — if Claude adds a section, rule, or paragraph that Vasily did not ask for, Claude says so explicitly and proposes to keep, modify, or remove. Never sneak in additions.
...
- **Is this skill for Claude (this chat) or for Claude Code or both?** Different surfaces have different conventions. Vasily's existing skills mostly target Claude (with prompts produced for Claude Code). Be explicit about the surface.
...
| **Claude — this chat** (reasoning layer) | Senior partner | Researches best practices, drafts the skill iteratively, runs multi-perspective review, maintains honesty about additions |
| **Filesystem** | Storage | The skill folder containing `SKILL.md` and optionally `references/`, `scripts/`, `assets/` |

### Autonomy principle

Same as in `bug-hunting` Section 1.5: Claude works as autonomously as possible. For skill creation specifically:
...
**Standard path for Vasily's skills:** `/mnt/skills/user/<skill-name>/SKILL.md`

This is where new skills are saved by default. Other locations exist:
...
When Vasily saves a finalized skill — confirm the path matches `/mnt/skills/user/<skill-name>/SKILL.md` so future sessions discover it correctly.
...
The working delivery method — confirmed to trigger the "Save" button in Vasily's interface — is a `.skill` package file:
...
Claude writes the first version using the mandatory structure (Section 7). The draft includes:
...
|---|---|---|
| **Stakeholder** | Vasily — **simulated by Claude** (see below) | Will Vasily actually use this? Is it written in his idiom? Does it solve the brief from Phase 1? Does it fit his workflow (chat-based discussion, MCP autonomy, manager role)? |
| **Technical** | Senior Claude — skill engineer | Description triggers correctly (not too vague, not too narrow)? Cross-references correct and specific (section numbers, not just file names)? Size within 500-line guideline? File header present? Anti-patterns absent? |
...
If Claude added anything during drafting that was not in the Phase 1 brief — disclose explicitly. See Section 15 (Honesty about additions).
...
5. **Workflow Architecture (where relevant)** — three-layer model (Vasily / Claude / Claude Code or Filesystem) when the skill involves coordination between Vasily, Claude, Claude Code, or external systems. Skip if the skill is purely a reasoning/format standard with no actor coordination. Pattern from `bug-hunting` Section 1.5.
...
- **Examples** — concrete cases illustrating the rules. Anthropic's official guide recommends including reasoning chains, not just outputs, so Claude has a model of how to think through the skill's domain.
...
Claude **under-triggers skills by default** — it errs toward not loading even when the skill is relevant. Anthropic's official guidance: descriptions should be slightly aggressive, explicitly listing synonyms and edge cases.
...
|---|---|---|
| `disable-model-invocation: true` | Skill can only be invoked manually (`/skill-name`), Claude won't auto-trigger | For skills that perform actions with consequences (deploy, rollback, payment operations) where unintended invocation is unsafe |
...
| `compatibility: <runtime>` | Marks the skill as runtime-specific (Claude Code, Claude.ai, etc.) | When the skill uses platform-specific features |
...
1. **Generate test queries.** Claude generates 6–10 queries:
...
2. **Run the test.** For each query, in a fresh conversation context, ask Claude: "Given this query: [Q]. Would you load skill `<skill-name>`? Cite the description text that supports your decision."
3. **Score results.**
...
This is a chat-based validation, not subagent-based (Vasily's workflow doesn't use subagents). Vasily and Claude run it together in Phase 6.
...
If Claude, during drafting or editing a skill, adds content that Vasily did not explicitly ask for — Claude must disclose this before presenting the result.
...
After presenting the skill draft, Claude posts a separate message in the chat (not as a comment inside `SKILL.md` — that pollutes the file) titled "Самовольные добавления", listing:
- What was added
- Why Claude thought it was needed
...
- Would a future Claude (or Vasily after 3 months) easily find the right section?
...
Splitting or merging without Vasily's explicit approval is forbidden — even if Claude is convinced it's the right move. Propose, get approval, execute.
...
Deprecating a skill Claude itself helped create is uncomfortable — there's a temptation to defend it. Resist. If the staleness test shows the skill no longer earns its place — say so plainly.
...
3. **Compare on explicit criteria** (defined in advance, not after seeing outputs):
   ...
   - Did Claude reach for the skill at the right moments? (For default-trigger skills)
...
8. **Ignoring the under-triggering tendency** — if the description is "polite" and minimal, Claude will under-load it. Be slightly aggressive.
...
- **`prompt-writing-standard`** — every change to a skill is delivered as a Claude Code prompt following that standard. Phase 6 (Finalization) of this workflow produces the prompt. Multi-perspective review pattern (`prompt-writing-standard` Section 2 Step 9) is the model for this skill's Phase 4.
