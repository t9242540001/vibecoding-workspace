...
|---|---|---|
| **Vasily** (decision layer) | Manager / visionary | Approves plans, copies prompts to Code Agent, executes commands that require interactive input or sudo, confirms deploys, validates fixes |
| **AI model / orchestrator** (reasoning layer) | Senior technical partner | Investigates autonomously via MCP tools, forms hypotheses, generates prompts for Code Agent, maintains the investigation log, makes recommendations |
| **Code Agent** (executor layer) | Coding agent | Reads/writes repository files, runs tests, makes commits — strictly via prompts following `prompt-writing-standard` |
| **VDS / production** | Target environment | Reached primarily by the AI model / orchestrator through available file connectors or MCP tools; reached by Vasily when tools cannot |

### Autonomy principle

**The AI model / orchestrator does the work. Vasily approves, copies, executes commands that require him.**

The AI model / orchestrator must attempt the task as autonomously as possible by using available tools before requesting Vasily's involvement:

- **Files in the repository** → read via available repository/VDS file connectors or MCP tools, do not ask Vasily to paste file contents
- **Knowledge base lookups** → use available knowledge-base tools or MCP connectors and read `knowledge/INDEX.md` + relevant files directly
- **Server-side files and logs** → read via available VDS/server file connectors or MCP tools wherever possible (log files, config files, databases via file access)
...
The AI model / orchestrator turns to Vasily only when:
...
When the AI model / orchestrator does ask Vasily for something, the request is specific: exact command to run, exact file to look at, exact question to answer. Never a vague "please check the logs".
...
| Prompts for Code Agent, RULE comments in code, `decisions.md` entries, `rules.md` entries | English | Per `prompt-writing-standard` Section 4 — repository artifacts stay in English |
...
This skill is **mandatory** when any of the following observable signals appear. The AI model / orchestrator switches to bug-hunting mode unilaterally upon detecting any of them, and informs Vasily explicitly: "Перехожу в режим bug-hunting по триггеру X — далее по протоколу."

### Trigger 1 — Two failed fix attempts on the same bug

Two prompts have been sent to Code Agent with the intention of fixing the same bug, and after both, the bug still reproduces by the original scenario. The third prompt addressing the same bug MUST be preceded by activating this skill — regardless of how confident the next approach feels. This matches the system instruction's "third attempt with same method is ineffective" rule.
...
Not every failed fix deserves the full protocol. If after the first failed fix the cause is mechanically obvious (typo in a variable name, wrong import path, missed null check now visible in the diff) — The AI model / orchestrator may apply Lightweight Mode.
...
Answer all of these explicitly before moving on. If any answer is "I don't know" — that is the first thing to find out, not assume. The AI model / orchestrator attempts to answer each question autonomously via MCP tools before turning to Vasily.

- What exactly broke? State the symptom in one sentence — what the user sees vs what they expected.
- When did it last work? (sets the time boundary for bisection) — The AI model / orchestrator can check via `git log` through file MCP.
- What changed since then? Commits, configs, env vars, dependencies, data, external services. The AI model / orchestrator reads `git log`, recent commit diffs, deploy history via MCP.
...
- Is this a new bug or a recurring one?
  - **The AI model / orchestrator proactively runs `conversation_search`** with keywords from the symptom (error message fragment, file name, symptom description) before asking Vasily. Past investigations of similar bugs are highly relevant — they may contain the answer or rule out hypotheses cheaply.
  - **the AI model / orchestrator reads `knowledge/decisions.md`** via file MCP to check for previous occurrences of this symptom or related ones.
- Does this match any pattern in `knowledge/rules.md` or any `RULE:` comment in the affected files? The AI model / orchestrator reads via MCP.
...
- Confirm: does it reproduce locally / on staging / in prod? The AI model / orchestrator can check files and logs across environments via MCP.

### Branch B — Bug is intermittent (heisenbug)

If the bug doesn't reproduce reliably:

- Run the failing scenario N times (typically 20–100), record success/failure ratio. For server-side bugs the AI model / orchestrator can request Vasily to run a loop, or write a small reproduction script as a Code Agent prompt.
...
- Add structured logging with correlation IDs at the suspected paths (delivered as a Code Agent prompt, deployed, then logs read back via available VDS/server file connectors or MCP tools)
...
- **Read-only access to production data — never write or experiment in prod via MCP, only read.** Any changes go through a Code Agent prompt and Vasily-approved deploy.
...
Cheap and informative first. Don't read code until you've read the error. Don't google until you've formed your own hypothesis. **The AI model / orchestrator executes each step autonomously via MCP wherever possible**, asking Vasily only when the source is genuinely outside MCP reach.

| # | Source | Why this position | How the AI model / orchestrator accesses it |
|---|---|---|---|
| 1 | Full error message + stack trace | Often contains the answer. Read in full, never paraphrased. | Read from logs via available VDS/server file connectors or MCP tools, or from chat if Vasily already pasted it |
...
| 2 | `knowledge/decisions.md`, `knowledge/rules.md` | Has this happened before? Most under-used source. | Read via available file connectors or MCP tools |
...
| 4 | Server logs (`pm2 logs`, `journalctl`, application log files) | Reality of what the system did. | Read log files directly via available VDS/server file connectors or MCP tools. If a live `pm2 logs` stream is needed — request Vasily to run and paste the relevant window. |
...
| 6a | **Walk through the failing input manually** | For data-processing bugs: trace one specific failing input through the code mentally, or have Code Agent add temporary logging at each transformation step | Combination of AI model / orchestrator reading code via MCP and writing a logging prompt |
...
- Correct project / branch in Code Agent (per system instruction reminder)?
...
How tested: [The AI model / orchestrator reads file X / AI model / orchestrator writes Code Agent prompt to add logging / Vasily runs command Y / etc.]
```
...
In Vasily's workflow, bisection is delivered as a Code Agent prompt: "Perform git bisect between commit X (known good) and HEAD (known bad), testing with [reproduction command from Phase 2]. Report the first bad commit." The AI model / orchestrator formulates the prompt and explains the bisect concept; Code Agent executes; Vasily approves and forwards results. Logarithmic search — even 1000 commits collapses to 10 tests.

### 5.5. Change one thing at a time (Agans Rule #5)

If you change two things and the bug disappears, you don't know which change helped. The "fix" may unravel an hour later because the second change was masking a third problem. Each Code Agent prompt changes one thing.

### 5.6. Investigation log — The AI model / orchestrator maintains it inline in the chat

The AI model / orchestrator maintains the investigation log in the chat in real time, in this format:
```
...
This check is *complementary* to the REGRESSION SHIELD block inside the fix prompt (per `prompt-writing-standard` Section 3). The shield prevents Code Agent from breaking neighbors *during* the fix; this check verifies *after* execution that nothing actually broke. Both are mandatory, neither replaces the other.
...
The test should reproduce the bug as documented in Phase 2 — and pass after the fix. Test creation is delivered as a separate Code Agent prompt (see Section 8 — sequence of prompts).

### 6.6. RULE anchor in code

Add a `RULE:` comment next to the vulnerable code (per `code-markup-standard` Section 8). The anchor is the anti-regression mechanism at the code level — Code Agent sees it on every future edit.
...
1. The fix prompt has been executed by Code Agent and merged
...
11. **Asking Vasily to do MCP-accessible work** — if the file or log can be read via MCP, the AI model / orchestrator reads it and does not ask Vasily to paste it (per Section 1.5 autonomy principle)
