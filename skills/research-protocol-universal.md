...
The trigger test is four self-directing questions, asked before answering any non-trivial decision-support request. If **any one** triggers, AI model / orchestrator proposes research before proceeding.
...
**One trigger is enough.** AI model / orchestrator does not need to count how many questions fire — one clear yes is the threshold.
...
AI model / orchestrator never launches research unilaterally — the cost to Vasily (waiting, reading) is real. When a trigger fires, AI model / orchestrator proposes in one phrase, names the trigger, and waits for one-word approval:

> "Вижу триггер [короткая формулировка какого именно вопроса] — предлагаю исследование перед ответом. Запускать?"

If Vasily approves, AI model / orchestrator runs the full protocol. If Vasily says no, AI model / orchestrator proceeds directly with a brief answer noting the skipped step.
...
| **AI model / orchestrator** (reasoning layer) | Senior partner | Runs all four phases: data gathering, team discussion, critical phase, final report |
...
AI model / orchestrator conducts the research autonomously once launched. Vasily is not asked intermediate questions during phases — intermediate input breaks the critical-phase dynamic (consensus forms around what Vasily seems to want). Vasily sees the completed report and responds once.

Exception: if during phase 1 or 2 AI model / orchestrator discovers the original question was misframed (e.g. Vasily asked "which payment provider" but the real question is "do we need payments at all yet"), AI model / orchestrator pauses and reports this to Vasily before continuing.
...
From the answer, AI model / orchestrator picks sources — **in this order of priority**:
...
- **Disagreements are named explicitly.** Where positions diverge, AI model / orchestrator states where and on what basis. Where positions converge, AI model / orchestrator states that too ("team converges on X").
...
This phase exists because phases 1 and 2 tend to produce a converging consensus. The job of phase 3 is to attack that consensus. AI model / orchestrator switches role: stops looking for the best answer, starts looking for reasons to reject the tentative recommendation.
...
AI model / orchestrator generates this block pre-filled; Vasily copies it into the project's `decisions.md` if he applies the recommendation. If Vasily rejects the recommendation — no entry.
...
These are the specific failure modes this skill exists to prevent. If AI model / orchestrator catches itself doing any of them — stop, return to the phase that was skipped.
...
11. **Inflating the research proposal into a long pitch** — the proposal is one phrase naming the trigger. If AI model / orchestrator needs three paragraphs to justify why research is needed, that pitch competes with the research itself and trains Vasily to dismiss the skill as overhead.
...
- **`prompt-writing-standard`** — research often produces, as its result, a Code Agent prompt based on the approved decision. That prompt still follows `prompt-writing-standard` Section 2 (Context / Task / Regression Shield / Acceptance Criteria). This skill stops at the research report; prompt-writing takes over from there.
