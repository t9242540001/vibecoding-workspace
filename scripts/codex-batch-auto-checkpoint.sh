...
cat > "$TASK_FILE" <<EOF
Run Codex batch $BATCH_ID according to standards/codex-batch-execution-standard.md.
Do not use Claude Routines.
Do not run deploy/server/secrets actions without separate explicit approval.
After completion, stop and print a short report: changed files, checks, and stop conditions.
EOF
